...
try:
    QTTR = int(os.environ.get('QUEUEIT_TTR', beanstalkc.DEFAULT_TTR))
except ValueError:
    LOG.error("Incorrect value for QUEUEIT_TTR. Using '%s' instead" % beanstalkc.DEFAULT_TTR)
...
def _get_qconnection(host, port):
    try:
        return beanstalkc.Connection(host=host, port=port, parse_yaml=True)
...
def qwrapper(tube_in, tube_out, worker_cmd):
    ...
    LOG.info(u"QHOST: %s, QPORT %s, TTR %s, PRIORITY %s, Watching queues: %s" % (
        QHOST, QPORT, QTTR, QPRIORITY, qconn_in.watching()))
    while True:
        job = qconn_in.reserve(timeout=QTIMEOUT)
        if not job:
...
def main():
    try:
        ...
        args = sys.argv[1:]
        if COMMAND == 'queueit' or COMMAND == '__init__.py':
            if len(sys.argv) == 1:
                print("Usage:")
        ...
        if COMMAND == 'q-get':
            ...
            if len(args) == 1:
                ...
                print("Usage: %s <queue> [<message>, <message>, ...]\n Message body could be sent trough STDIN" % (
                    COMMAND))
                sys.exit(1)
        elif COMMAND == 'q-kick':
